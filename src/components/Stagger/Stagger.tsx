...
import type { BaseAnimationBuilder, EntryExitAnimationFunction, Keyframe } from 'react-native-reanimated'
import Animated, {
        ...
        return (
          <Animated.View
            ...
            entering={(entering() as ComplexAnimationBuilder)
              .delay(
                initialEnteringDelay +
                  (enterDirection === 1 ? index * stagger : (React.Children.count(children) - index) * stagger),
              )
              .duration(duration)
            ...
            exiting={(exiting() as ComplexAnimationBuilder)
              .delay(
                initialExitingDelay +
                  (exitDirection === 1 ? index * stagger : (React.Children.count(children) - index) * stagger),
              )
              .duration(duration)
              .withCallback((finished) => {
                'worklet'
